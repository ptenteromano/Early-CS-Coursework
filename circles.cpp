...
public:
	...
	Circles(float r, int x, int y);			// Constructor for all inputs
	Circles(float r);				//added constructor for radius input
	Circles(int x, int y);				//added constructor for center input
	Circles();					// Default constructor 
	~Circles();					//added Destructor
private:
	float	radius;
	int	center_x;
	int	center_y;
...
int main()
{
	...
	cout<<"The circumeference of the circle is "<<sphere2.findCircumference()<<endl;	
...
//Exercise 2: define a constructor using partial variable input
...
//Exercise 3: a fourth constructor allows either no input, all input, only radius, or only center
...
}//the destructor is printed four times, one for each object created - it is releasing memory
